...
export default function ColorPaletteGenerator() {
  ...
  const updateURL = useCallback(
    (colors: Color[]) => {
      const hexCodes = colors
        ...
        .join('-');
      router.push(`/colors/${hexCodes}`, { scroll: false });
    },
    [router]
  );
  ...
  // Initialize from URL or generate new palette
  useEffect(() => {
    const matches = pathname.match(/\/colors\/([a-fA-F0-9-]+)/);

    if (matches && matches[1]) {
      const hexCodes = matches[1].split('-').map((hex) => `#${hex}`);
