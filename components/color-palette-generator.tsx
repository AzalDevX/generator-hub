...
:root {
${colors
  .map((color) => `  --${color.name.replace(/\s+/g, '-')}: ${color.hex};`)
  .join('\n')}
}`.trim(),
  },
  {
...
${colors
  .map(
    (color) => `        '${color.name.replace(/\s+/g, '-')}': '${color.hex}',`
  )
  .join('\n')}
      }
    }
...
export default function ColorPaletteGenerator() {
  ...
  const downloadPalette = (format: ExportFormat) => {
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to download the palette.',
        variant: 'error',
      });
    }
  };
  ...
  const shareToTwitter = () => {
    try {
      const url = shareUrl();
      const text = `Check out this color palette I created with Generators Hub! 🎨\n\n${colors
        .map((c) => c.hex)
        .join(' ')}`;
      window.open(
  ...
  }, [colors.length, generatePalette]); // Only run on mount and when colors.length changes
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center space-x-4">
            <select
              value={scheme}
              onChange={(e) =>
                handleSchemeChange(e.target.value as ColorScheme)
              }
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-2 w-48 py-2 bg-discord-secondary rounded-lg shadow-lg border border-discord-tertiary opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                <button
                  ...
                  className="w-full px-4 py-2 text-left text-discord-text-secondary hover:text-discord-text-primary hover:bg-discord-tertiary transition-colors flex items-center gap-2">
